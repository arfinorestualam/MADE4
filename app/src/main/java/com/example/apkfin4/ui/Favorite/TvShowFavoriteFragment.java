...
import static com.example.apkfin4.db.FavHelper.getTvFavorite;
...
public class TvShowFavoriteFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View v, @Nullable Bundle bundle) {
        ...
        callback = new FavoritesCallback() {
            ...
            public void postExecute(Cursor cursor) {
                ArrayList<Favorite> arrayList = getTvFavorite(cursor);
                if (arrayList.size() > 0) {
            ...
            HandlerThread handler = new HandlerThread("Observer");
            handler.start();
    ...
    private static class FavAsync extends AsyncTask<Void,Void,Cursor> {
        ...
        FavoritesCallback callback;
            public DataObserver(Handler handler, Context context, FavoritesCallback callback) {
                super(handler);
